...
namespace Assets.Scripts.Penpenpng
{
	...
	class State2 : State
	{
		...
		const int AbsPhiSize = 7;
		//phiの符号。0が非正で1が正
		public int PhiSign { get; private set; }
		const int PhiSignSize = 2;
		//赤1と緑2の距離を0～1で表現したもの
		public int Distance { get; private set; }
		const int DistanceSize = 5;

		public override string Identifier()
		{
			return string.Format("s2:{0}:{1}:{2}:{3}:{4}", AbsTheta, ThetaSign, AbsPhi, PhiSign, Distance);
		}
		...
		//区間の区切り点
		static readonly int[] ThetaKnots = new int[AbsThetaSize - 1] { 10, 20, 30, 60, 90, 120 };
		static readonly int[] PhiKnots = new int[AbsPhiSize - 1] { 10, 20, 30, 60, 90, 120 };
		static readonly int[] DistanceKnots = new int[DistanceSize - 1] { 30, 60, 90, 120 };
	...
	class Action2 : Action
	{
		...
		public override string Identifier()
		{
			return string.Format("a2:{0}:{1}:{2}", Yaw, Speed, Shoot);
		}
